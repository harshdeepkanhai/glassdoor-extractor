...
let file = ['OYO-Software-Developer.csv'];
for (let i = 0; i < file.length; i++) {
  fs.createReadStream(file[i])
    .pipe(csv({ separator: ';' }))
    .on('data', data => {
      urls.push(data);
    })
    .on('end', async () => {
      try {
        ...
        for (let i = 0; i < urls.length; i++) {
          ...
          let answer = `I think the key in generic questions like this is to be careful to cover the fundamentals, and to be familiar with all the followups so you're prepared for whatever they throw at you. Maybe do a mock interview with a ${company} ${role} expert on PrepTick to get a real-world answer? They give lots of guidance and pro tips on how to deal with this kind of stuff. https://www.preptick.com/practice-interviews-swe`;
